#ifndef _NCK_interflowsw_H_
#define _NCK_interflowsw_H_
 ...
 */
int nck_interflow_sw_create_enc(struct nck_encoder *encoder, struct nck_timer *timer, void *context, nck_opt_getter get_opt);
/**
 ...
 * @get_opt: a function to extract a configuration value from the context
 */
int nck_interflow_sw_create_dec(struct nck_decoder *decoder, struct nck_timer *timer, void *context, nck_opt_getter get_opt);
/**
 ...
 * @get_opt: a function to extract a configuration value from the context
 */
int nck_interflow_sw_create_rec(struct nck_recoder *recoder, struct nck_timer *timer, void *context, nck_opt_getter get_opt);

struct nck_interflow_sw_enc *nck_interflow_sw_enc(uint32_t symbols, uint32_t symbol_size, struct nck_timer *timer, const struct timeval *timeout);
/**
 ...
 */
struct nck_interflow_sw_dec *nck_interflow_sw_dec(uint32_t symbols, uint32_t symbol_size, struct nck_timer *timer, const struct timeval *timeout, const char *algorithm);
 ...
 */
void nck_interflow_sw_enc_set_memory(struct nck_interflow_sw_enc *encoder, uint32_t memory);
/**
...
/**
 * Set the node id of the encoder
 *
 * In order to identify packets from different sources in interflow network coding, a unique identifier for each source has to be assigned.
 * @decoder: decoder structure to configure
 * @node_id: node id of the source
 ...
 *
 * The number of nodes is required to enable unique packet numbers for all nodes
 * @decoder: decoder structure to configure
 * @n_nodes: node id of the source

 */
void nck_interflow_sw_enc_set_n_nodes(struct nck_interflow_sw_enc *encoder, uint32_t n_nodes);

NCK_ENCODER_API(nck_interflow_sw)
NCK_DECODER_API(nck_interflow_sw)
NCK_RECODER_API(nck_interflow_sw)

#ifdef __cplusplus
} /* extern "C" */
#endif

#endif /* _NCK_interflowsw_H_ */
