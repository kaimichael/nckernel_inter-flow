...
NCK_ENCODER_IMPL(nck_gack, NULL, NULL, NULL)

EXPORT
struct nck_gack_enc *nck_gack_enc(krlnc_encoder_factory_t factory)
{
	struct nck_gack_enc *result;
	uint32_t block_size;
...
EXPORT
void nck_gack_enc_free(struct nck_gack_enc *encoder)
{
...
EXPORT
int nck_gack_enc_has_coded(struct nck_gack_enc *encoder)
{
	return !encoder->empty;
}

EXPORT
int nck_gack_enc_full(struct nck_gack_enc *encoder)
{
	return encoder->full;
}

EXPORT
int nck_gack_enc_complete(struct nck_gack_enc *encoder)
{
	return encoder->complete;
}

EXPORT
void nck_gack_enc_flush_coded(struct nck_gack_enc *encoder)
{
	encoder->full = 1;
}

EXPORT
int nck_gack_enc_put_source(struct nck_gack_enc *encoder, struct sk_buff *packet)
{
	if (encoder->complete) {
		encoder->generation++;
		encoder->rank = 0;
...
EXPORT
int nck_gack_enc_get_coded(struct nck_gack_enc *encoder, struct sk_buff *packet)
{
...
EXPORT
int nck_gack_enc_put_feedback(struct nck_gack_enc *encoder, struct sk_buff *packet)
{
	...
	if (generation == encoder->generation && rank == encoder->rank) {
		encoder->complete = 1;
		encoder->empty = 1;
		encoder->full = 0;
	}
