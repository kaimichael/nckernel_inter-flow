...
NCK_DECODER_IMPL(nck_gsaw, nck_gsaw_dec_debug, NULL, NULL)

EXPORT
char *nck_gsaw_dec_debug(void *dec)
{
	static char debug[1024];
	struct nck_gsaw_dec *decoder = dec;
...
EXPORT
void nck_gsaw_dec_free(struct nck_gsaw_dec *decoder)
{
...
EXPORT
int nck_gsaw_dec_has_source(struct nck_gsaw_dec *decoder)
{
...
EXPORT
int nck_gsaw_dec_complete(struct nck_gsaw_dec *decoder)
{
	return decoder->index == decoder->symbols;
}

EXPORT
void nck_gsaw_dec_flush_source(struct nck_gsaw_dec *decoder)
{
	decoder->flush = 1;
...
EXPORT
int nck_gsaw_dec_put_coded(struct nck_gsaw_dec *decoder, struct sk_buff *packet)
{
	uint32_t generation;
	uint32_t rank;
...
EXPORT
int nck_gsaw_dec_get_source(struct nck_gsaw_dec *decoder, struct sk_buff *packet)
{
	return kodo_get_source(decoder->coder, packet, decoder->buffer, &decoder->index, decoder->flush);
}

EXPORT
int nck_gsaw_dec_get_feedback(struct nck_gsaw_dec *decoder, struct sk_buff *packet)
{
	skb_reserve(packet, 6);
	skb_push_u16(packet, decoder->feedback_rank);
...
EXPORT
int nck_gsaw_dec_has_feedback(struct nck_gsaw_dec *decoder)
{
