...
struct nck_sw_rec {
	nck_sw_rec(coder_t coder, int ord) :
...
char *nck_sw_rec_describe_packet(void *recoder, struct sk_buff *packet);
struct nck_stats *nck_sw_rec_get_stats(void *recoder);

NCK_RECODER_IMPL(nck_sw, NULL, nck_sw_rec_describe_packet, nck_sw_rec_get_stats)
...
static void recoder_timeout_flush(struct nck_timer_entry *entry, void *context, int success)
{
	...
	if (success) {
		struct nck_sw_rec *recoder = (struct nck_sw_rec *)context;

		recoder->stats.s[NCK_STATS_TIMER_FLUSH]++;

		if (!nck_sw_rec_has_source(recoder)) {
			nck_sw_rec_flush_source(recoder);
		}
		if (!nck_sw_rec_has_coded(recoder)) {
			nck_sw_rec_flush_coded(recoder);
		}
	}
}
...
struct nck_sw_rec *nck_sw_rec(uint32_t symbols, uint32_t symbol_size, struct nck_timer *timer, const struct timeval *timeout)
{
	...
	struct nck_sw_rec *result = new struct nck_sw_rec(factory.build(), ord);
...
EXPORT
void nck_sw_rec_free(struct nck_sw_rec *recoder)
{
...
EXPORT
int nck_sw_rec_has_source(struct nck_sw_rec *recoder)
{
	return recoder->queue_length != recoder->queue_index || recoder->has_source;
}

EXPORT
int nck_sw_rec_complete(struct nck_sw_rec *recoder)
{
	UNUSED(recoder);
...
static void move_to_next_source(struct nck_sw_rec *recoder)
{
	auto coder = recoder->coder;
...
EXPORT
void nck_sw_rec_flush_coded(struct nck_sw_rec *recoder)
{
	auto coder = recoder->coder;
...
EXPORT
void nck_sw_rec_flush_source(struct nck_sw_rec *recoder)
{
	auto coder = recoder->coder;
 ...
 */
static inline void nck_sw_rec_apply_forward_window(struct nck_sw_rec *recoder, uint32_t distance)
{
	auto coder = recoder->coder;
 ...
 */
static void nck_sw_rec_put_coded_consume_old(struct nck_sw_rec *recoder,
					     uint32_t sequence,
					     uint32_t symbols)
{
	auto coder = recoder->coder;
...
EXPORT
int nck_sw_rec_put_coded(struct nck_sw_rec *recoder, struct sk_buff *packet)
{
	...
	packet_no = ntohs(sw_coded_packet->packet_no);
	 ...
	 * to queue for later
	 */
	nck_sw_rec_put_coded_consume_old(recoder, header.sequence, symbols);
	...
	nck_sw_rec_apply_forward_window(recoder, coder->sequence_number() - previous_seqno);

	/* TODO: only flush if target rate > 0? The current behaviour is that even
	 * "silent" recoders transport the flush. */
	if (sw_coded_packet->flags & SW_CODED_PACKET_FLUSH &&
		...
		recoder->flush_next = sw_coded_packet->flags & SW_CODED_PACKET_FLUSH;
...
EXPORT
int nck_sw_rec_has_coded(struct nck_sw_rec *recoder)
{
 ...
 */
static bool nck_sw_check_zero_coefficients(coder_t coder, uint8_t *payload)
{
...
EXPORT
int nck_sw_rec_get_coded(struct nck_sw_rec *recoder, struct sk_buff *packet)
{
	assert(nck_sw_rec_has_coded(recoder));
	...
	skb_reserve(packet, sizeof(*sw_coded_packet));
	...
	if (nck_sw_check_zero_coefficients(coder, payload)) {
	...
	sw_coded_packet = (struct sw_coded_packet *)skb_push(packet, sizeof(*sw_coded_packet));
	memset(sw_coded_packet, 0, sizeof(*sw_coded_packet));
	sw_coded_packet->packet_type = SW_PACKET_TYPE_CODED;
	sw_coded_packet->order = recoder->order;
	sw_coded_packet->packet_no = htons(recoder->last_packet_no);

	if (recoder->flush_next) {
		sw_coded_packet->flags |= SW_CODED_PACKET_FLUSH;
		sw_coded_packet->packet_no = htons(recoder->flush_packet_no);
...
EXPORT
char *nck_sw_rec_describe_packet(void *rec, struct sk_buff *packet)
{
	struct nck_sw_rec *recoder = (struct nck_sw_rec*)rec;

	return nck_sw_common_describe_packet(packet, recoder->coder->symbols());
}

EXPORT
struct nck_stats *nck_sw_rec_get_stats(void *rec)
{
	struct nck_sw_rec *recoder = (struct nck_sw_rec*)rec;
...
EXPORT
int nck_sw_rec_get_source(struct nck_sw_rec *recoder, struct sk_buff *packet)
{
	auto coder = recoder->coder;
	uint32_t symbol_size = coder->symbol_size();
 ...
 */
static inline void nck_sw_rec_schedule_retransmission(struct nck_sw_rec *recoder, uint8_t *data, uint32_t feedback_seqno)
{
	auto coder = recoder->coder;
	int symbols = coder->symbols();
	int rank = coder->rank();
 ...
 */
static inline void nck_sw_rec_apply_feedback(struct nck_sw_rec *recoder, uint8_t *data, uint32_t feedback_seqno)
{
	auto coder = recoder->coder;
	int symbols = coder->symbols();
	uint32_t local_seqno = coder->sequence_number();
	...
	if (seqno_dist > 0) {
		...
		nck_sw_rec_put_coded_consume_old(recoder, feedback_seqno, symbols);
		nck_sw_rec_apply_forward_window(recoder, feedback_seqno);
...
EXPORT
int nck_sw_rec_put_feedback(struct nck_sw_rec *recoder, struct sk_buff *packet)
{
	auto coder = recoder->coder;
	int symbols = coder->symbols();
	...
	skb_pull(packet, sizeof(*sw_feedback_packet));

	if (sw_feedback_packet->packet_type != SW_PACKET_TYPE_FEEDBACK)
		return -1;

	feedback_no = ntohs(sw_feedback_packet->feedback_no);
	feedback_sequence = ntohl(sw_feedback_packet->sequence);
	...
	nck_sw_rec_apply_feedback(recoder, packet->data, feedback_sequence);

	/* copy the feedback to send it out again */
	memcpy(&recoder->feedback_buffer[0], sw_feedback_packet, recoder->feedback_size);
	recoder->last_feedback_no = feedback_no;
	recoder->has_feedback = 1;
	nck_trigger_call(&recoder->on_feedback_ready);

	nck_sw_rec_schedule_retransmission(recoder, packet->data, feedback_sequence);

	return 0;
}

EXPORT
int nck_sw_rec_get_feedback(struct nck_sw_rec *recoder, struct sk_buff *packet)
{
...
EXPORT
int nck_sw_rec_has_feedback(struct nck_sw_rec *recoder)
{
