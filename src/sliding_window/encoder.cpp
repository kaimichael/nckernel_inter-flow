...
struct nck_sw_enc {
	nck_sw_enc(coder_t coder, int ord) :
...
char *nck_sw_enc_debug(void *encoder);
char *nck_sw_enc_describe_packet(void *encoder, struct sk_buff *packet);
struct nck_stats *nck_sw_enc_get_stats(void *encoder);

NCK_ENCODER_IMPL(nck_sw, nck_sw_enc_debug, nck_sw_enc_describe_packet, nck_sw_enc_get_stats)
...
EXPORT
void nck_sw_enc_set_memory(struct nck_sw_enc *encoder, uint32_t memory_size)
{
...
static void encoder_timeout_flush(struct nck_timer_entry *entry, void *context, int success)
{
	...
	if (success) {
		struct nck_sw_enc *encoder = (struct nck_sw_enc *)context;
...
EXPORT
struct nck_sw_enc *nck_sw_enc(uint32_t symbols, uint32_t symbol_size, struct nck_timer *timer,
		const struct timeval *timeout)
{
	...
	struct nck_sw_enc *result = new struct nck_sw_enc(factory.build(), ord);
...
EXPORT
void nck_sw_enc_free(struct nck_sw_enc *encoder)
{
...
EXPORT
int nck_sw_enc_has_coded(struct nck_sw_enc *encoder)
{
...
EXPORT
int nck_sw_enc_full(struct nck_sw_enc *encoder)
{
...
EXPORT
int nck_sw_enc_complete(struct nck_sw_enc *encoder)
{
	if (_has_coded(encoder)) {
...
static void nck_sw_enc_free_symbols(struct nck_sw_enc *encoder)
{
	auto coder = encoder->coder;
...
EXPORT
void nck_sw_enc_flush_coded(struct nck_sw_enc *encoder)
{
	auto coder = encoder->coder;
	...
	if (encoder->flush_attempts <= 0) {
		...
		nck_sw_enc_free_symbols(encoder);
		return;
	}
...
EXPORT
int nck_sw_enc_put_source(struct nck_sw_enc *encoder, struct sk_buff *packet)
{
	auto coder = encoder->coder;
...
EXPORT
int nck_sw_enc_get_coded(struct nck_sw_enc *encoder, struct sk_buff *packet)
{
	...
	if (encoder->feedback_only_on_repair) {
		if (repair)
			flags |= SW_CODED_PACKET_FEEDBACK_REQUESTED;
	} else {
		if (!_has_coded(encoder)) {
	...
	sw_coded_packet->order = encoder->order;
	sw_coded_packet->flags = flags;
	sw_coded_packet->packet_no = htons(encoder->packet_count);
...
EXPORT
char *nck_sw_enc_debug(void *enc)
{
	struct nck_sw_enc *encoder = (struct nck_sw_enc*)enc;
...
EXPORT
char *nck_sw_enc_describe_packet(void *enc, struct sk_buff *packet)
{
	struct nck_sw_enc *encoder = (struct nck_sw_enc*)enc;

	return nck_sw_common_describe_packet(packet, encoder->coder->symbols());
}

EXPORT
struct nck_stats *nck_sw_enc_get_stats(void *enc)
{
	struct nck_sw_enc *encoder = (struct nck_sw_enc*)enc;
 ...
 *  window
 */
static bool nck_sw_feedback_seqno_valid(uint32_t sequence,
					uint32_t encoder_sequence,
...
EXPORT
int nck_sw_enc_put_feedback(struct nck_sw_enc *encoder, struct sk_buff *packet)
{
	auto coder = encoder->coder;
	...
	feedback_packet_no = ntohs(sw_feedback_packet->packet_no);
	sequence = ntohl(sw_feedback_packet->sequence);
	first_missing = ntohl(sw_feedback_packet->first_missing);

	nck_trace(encoder, "pktno=%u fbno=%u seq=%u miss=%u",
			feedback_packet_no, ntohs(sw_feedback_packet->feedback_no),
	...
	if (!nck_sw_feedback_seqno_valid(sequence, encoder_sequence, rank))
		return -1;
