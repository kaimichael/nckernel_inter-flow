...
static char *nck_interflow_sw_common_describe_coded_packet(struct sk_buff *packet, int symbols)
{
	...
	if (packet->len < sizeof(*interflow_sw_coded_packet) + sizeof(kodo_header) + symbols)
		return (char *)"\"error\":\"too short coded packet\"";

	pos += snprintf(&debug[pos], CHK_ZERO(len - pos), "\"packetno\":% 5d, \"seqno\":% 5d, ",
			ntohs(interflow_sw_coded_packet->packet_no),
			ntohl(kodo_header->seqno));

	pos += snprintf(&debug[pos], CHK_ZERO(len - pos), "\"flags\": { \"feedback_requested\": %s, \"flush\": %s }, ",
			(interflow_sw_coded_packet->flags & INTERFLOW_SW_CODED_PACKET_FEEDBACK_REQUESTED) ? "true" : "false",
			(interflow_sw_coded_packet->flags & INTERFLOW_SW_CODED_PACKET_FLUSH) ? "true" : "false");
...
static char *nck_interflow_sw_common_describe_feedback_packet(struct sk_buff *packet, int symbols)
{
	...
	bitmap = (uint8_t *) (interflow_sw_feedback_packet + 1);

	if (packet->len != sizeof(*interflow_sw_feedback_packet) + DIV_ROUND_UP(symbols, 8))
	...
	pos += snprintf(&debug[pos], CHK_ZERO(len - pos), "\"packetno\":% 5d, \"seqno\":% 5d, \"feedback_no\": % 5d, \"feedback\":true, \"bitmap\":\"",
			ntohs(interflow_sw_feedback_packet->packet_no), ntohl(interflow_sw_feedback_packet->sequence), ntohs(interflow_sw_feedback_packet->feedback_no));
...
EXPORT
char *nck_interflow_sw_common_describe_packet(struct sk_buff *packet, int symbols)
{
	...
	switch (packet_type) {
	case INTERFLOW_SW_PACKET_TYPE_CODED:
		return nck_interflow_sw_common_describe_coded_packet(packet, symbols);
		break;
	case INTERFLOW_SW_PACKET_TYPE_FEEDBACK:
		return nck_interflow_sw_common_describe_feedback_packet(packet, symbols);
		break;
	}
